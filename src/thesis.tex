...
\usepackage{color}
%\usepackage[showerrors]{xcolor}

% Allows the widths of certain columns, rather than the inter column space, to
% be modified.
\usepackage{tabularx}

% Sse biblatex instead of bibtex for biliographies
...
% between \begin{comment} and the next \end{comment}.
\usepackage{verbatim}
...
% governs appearance of floats.
\usepackage{float}

% To display matlab2tikz plots
\usepackage{pgfplots}

% For plots produced by GNUplot
...
\usepackage{xparse} % for \NewDocumentCommand, \NewDocumentEnvironment
